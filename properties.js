...
const QUESTIONS = [
  ...
  ], customPrompt: '✏️ כתוב עיר:' },
  { code: 'ad', dbKey: 'address', check: d => !d.address, question: '🏠 כתובת?', buttons: [], freeText: true },
  { code: 'ty', dbKey: 'type', check: d => !d.type, question: '🏢 סוג נכס?', buttons: [
  ...
  { code: 'dl', dbKey: 'deal_type', check: d => !d.deal_type, question: '🔑 סוג עסקה?', buttons: [
    [{ text: 'השכרה', val: 'השכרה' }, { text: 'מכירה', val: 'מכירה' }]
  ]},
  { code: 'ar', dbKey: 'area', check: d => !d.area || d.area === 0, question: '📐 שטח (מ"ר)?', buttons: [], freeText: true, isNumber: true, unit: 'מ"ר',
    customPrompt: '✏️ שטח במ"ר:' },
  { code: 'pr', dbKey: 'price', check: d => (!d.price || d.price === 0) && (!d.monthly_rent || d.monthly_rent === 0),
    question: '💰 מחיר / שכ"ד חודשי?', buttons: [], freeText: true, isNumber: true,
    customPrompt: '✏️ מחיר (15000 / 5M / 80K):' },
  { code: 'fl', dbKey: 'floor', check: d => d.type !== 'קרקע לבנייה' && (d.floor === undefined || d.floor === null),
    question: '🏗️ קומה?', buttons: [
    [{ text: 'קרקע', val: '0' }, { text: '1', val: '1' }, { text: '2', val: '2' }],
    [{ text: '3+', val: 'CUSTOM' }]
  ], isNumber: true, customPrompt: '✏️ מספר קומה:' },
  { code: 'pk', dbKey: 'parking', check: d => false, question: '🅿️ חניות?', buttons: [
    [{ text: '0', val: '0' }, { text: '1', val: '1' }, { text: '2+', val: 'CUSTOM' }]
  ], isNumber: true, customPrompt: '✏️ מספר חניות:' },
  { code: 'st', dbKey: 'status', check: d => !d.status, question: '📊 סטטוס?', buttons: [
    [{ text: 'זמין', val: 'זמין' }, { text: 'תפוס', val: 'תפוס' }, { text: 'בתהליך', val: 'בתהליך' }]
  ]},
  { code: 'ex', dbKey: 'exclusivity', check: d => d.exclusivity === undefined, question: '🤝 בלעדיות?', buttons: [
  ...
  { code: 'ds', dbKey: 'description', check: d => !d.description, question: '📝 תיאור?', buttons: [], freeText: true },
];
...
function register(bot, sessions) {
  ...
  async function saveProperty(chatId, session) {
    ...
    try {
      const result = await crm.createProperty({
        ...
        parking: d.parking||0, elevator: d.elevator||0, description: d.description||'',
        exclusivity: d.exclusivity ? 1 : 0,
        has_tenant: d.has_tenant ? 1 : 0, monthly_rent: d.monthly_rent||0, annual_yield: d.annual_yield||0,
        land_use: d.land_use||'', owner_id: d.owner_id||null
      });
      if (result.id) bot.sendMessage(chatId, `✅ נכס נוסף ל-CRM! (${d.type} ב${d.city})`, { parse_mode: 'Markdown' });
  ...
  async function startFromText(chatId, text) {
    ...
    try {
      ...
      if (hasMissing) {
        ...
        showSummary(chatId, sessions[chatId]);
      }
    } catch (e) { bot.sendMessage(chatId, `❌ ${e.message}`); }
  }
  ...
  async function startEmpty(chatId) {
    delete sessions[chatId];
    sessions[chatId] = { mode: 'property', data: {}, skipped: [] };
